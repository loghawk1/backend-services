...
class WebhookHandler:
    ...
    async def initialize(self):
        ...
        try:
            ...
            masked_url = self.settings.redis_url.replace(self.settings.redis_url.split('@')[0].split('//')[1],
                                                         'xxx:xxx') if '@' in self.settings.redis_url else self.settings.redis_url
    ...
    async def extract_revision_data(self, revision_webhook_data: RevisionWebhookData) -> Optional[
        ExtractedRevisionData]:
    ...
    async def extract_wan_data(self, webhook_data: WebhookData) -> Optional[ExtractedWanData]:
        ...
        try:
            ...
            wan_data_for_model = {
                ...
                "request_timestamp": body.get("request_timestamp"),
                "source": body.get("source", "web_app"),  # Provide default if missing
                "version": body.get("version"),
                "idempotency_key": body.get("idempotency_key"),
                "callback_url": body.get("callback_url"),
                "webhook_url": body.get("webhookUrl"),
            ...
            # Validate that required fields are present (removed 'source' from required list)
            required_fields = [
                ("prompt", extracted.prompt),
                ("image_url", extracted.image_url),
                ("video_id", extracted.video_id),
            ...
            logger.info(f"EXTRACT: Source: {extracted.source}")
            logger.info(f"EXTRACT: Prompt length: {len(extracted.prompt)} chars")
            return extracted
    ...
    async def queue_revision_task(self, extracted_data: ExtractedRevisionData) -> str:
        ...
        try:
            ...
            task_data = {
                ...
                "user_id": extracted_data.user_id,
                "revision_request": extracted_data.revision_request[:100] + "..." if len(
                    extracted_data.revision_request) > 100 else extracted_data.revision_request,
                "type": "revision"
            }
