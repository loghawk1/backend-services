        ...
        for i, image_url in enumerate(scene_image_urls):
            try:
                ...
                handler = await asyncio.to_thread(
                    ...
                    arguments={
                        "prompt": f"{wan2_5_prompt},Engaging, yet natural movement. Subtle camera shifts like organic pans or gentle pushes. Focus on subject's actions with enhanced, but believable energy. Avoid overly cinematic or overly shaky effects. When animating the clean source image, apply the conversion-optimized UGC Low-Fi aesthetic filter. Set the video frame rate to 10 FPS using Posterize Time to achieve a deliberately unpolished, non-cinematic look. Aggressively add High Grain/Noise and enforce Low Contrast, simulating the texture of heavy H.264 social media compression and features mandatory UGC-style captions on screen",
                        "image_url": image_url,
                        "resolution": "480p",
